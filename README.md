...
После этого будем выбирать последний элемент с конца списка сортировки и делать Эйлеров обход начиная с него. В случае, если из вершины можно попасть в какую-то другую вершину, данная вершина добавляется в компоненту сильной связности. Как только это невозможно, компонента добавляется в список компонент графа.
## Завершение работы
